...
class FMMPlanner:
    def __init__(self, traversible, num_rots=36.0, step_size=5, scale=1):
        self.scale = scale
        if scale != 1.0:
    ...
    def get_short_term_goal2(self, state):
        ...
        mask = self._get_mask(dx, dy, scale)
    ...
    def get_next_action(self, state, stg, agent_orientation):
        ...
        angle_st_goal = math.degrees(
            math.atan2(stg_x - state[0], stg_y - state[1])
        )
    ...
    def _find_nearest_goal(self, goal):
        ...
        planner4 = FMMPlanner(traversible4)
